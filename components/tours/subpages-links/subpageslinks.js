...
export default function Links({ allLinks, level3links = null, title, current, level, bus }) {
  ...
  const getNameTo = allLinks.filter((item) => item.subpage === router.query.subpage);
      ...
      {!level3links && (
        <>
          <h2 className={`${styles.subtitle} block_title`}>
            ...
            {!bus && intl.formatMessage({ id: 'country.from_2' })}
          </h2>
      ...
      {level === 3 && (
        <>
          <h2 className={`${styles.subtitle} block_title`}>
            ...
            <span className="mark">{getNameTo.length && getNameTo[0].translations[0].name}</span>
            {intl.formatMessage({ id: 'country.from_2' })}
          </h2>
          <div className={styles.toursfrom_items}>
