...
import {
  useIsLastResult,
  useHasMoreServerPages,
  useSearchStatus,
  useSearchTimedOut,
  useSearchPageNumber,
} from 'store/searchStore';
import styles from './ContinueSearchButton.module.css';

export default function ContinueSearchButton({ onContinue }) {
  const isLastResult = useIsLastResult();
  const hasMore = useHasMoreServerPages();
  ...
  if (!hasMore) {
    return (
      <div className={styles.callout}>
        <FM id="progress.no_more_results" />
      </div>
    );
  }
  ...
  return (
    <button
      type="button"
      className={`${styles.btn} ${isSearching ? styles.btn_loading : ''} main_form_btn`}
      onClick={onContinue}
    ...
    >
      ...
      <span className={styles.labelStack}>
        <span>
          <FM id="progress.continue_search" />
        </span>
