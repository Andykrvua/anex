...
export default function UpWindow({ setModalIsOpen, modalIsOpen, cName, popupName, value }) {
  ...
  let lastCountry = '';
  let filterArr = getStaticData?.translations?.[0]?.filter_value?.map((item) => item?.filter_name);
            ...
            <div className={styles.input_wrapper}>
              ...
              {getUpPointList.active &&
                getUpPointList.list.map((item, i) => {
                  if (!filterArr?.includes(item.country) && !filterArr?.includes(item.name)) {
