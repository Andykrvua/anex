...
export const getStaticData = async (loc) => {
  const locale = languagesApi[loc];
  const url = `static_data?fields=tag_manager_id,google_site_ver,no_transport,captcha_enabled,google_satellite_map,logo,telegram_banner,telegram_banner_x2,telegram_banner_x_3,translations.filter_value,translations.name&deep[translations][_filter][languages_code][_eq]=${locale}&filter[status]=published`;
  return req(url);
};
