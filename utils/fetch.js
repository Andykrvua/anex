...
export const getPostsSlugs = async () => {
  const url = `posts?fields=slug,status&filter[status][_eq]=published&limit=-1`;
  return req(url);
};
...
export const getAPICountryList = async () => {
  const url = `api_countries?fields=date_created,slug,img,translations.name,translations.languages_code&filter[status][_eq]=published&limit=-1`;
  return req(url);
};
...
export const getAPICountryListSlugs = async () => {
  const url = `api_countries?fields=slug,status&filter[status][_eq]=published&limit=-1`;
  return req(url);
};
...
export const getCountrySlugsAndSubpagesSlugs = async (
  ...
  const url = `api_countries_subpage?fields=country_slug.slug,status,subpage_slug,subdistrict_slug,subsubpage,subsubpage_slug&filter[status]=published${filter}&limit=-1`;
  return req(url);
};

export const getSubpagesSlugsFromCountry = async (slug) => {
  const url = `api_countries_subpage?fields=subsubpage,district_from_cities,country_slug.slug,popular,img,translations.languages_code,translations.name,translations.br,isNewCity,is_district,status,subpage_slug,temp_from,temp_to&filter[country_slug][slug][_eq]=${slug}&filter[status]=published&limit=-1`;
  return req(url);
};
...
export const getAllCountriesForNav = async (loc) => {
  const locale = languagesApi[loc];
  const url = `api_countries?fields=slug,code,translations.languages_code,translations.name&deep[translations][_filter][languages_code][_eq]=${locale}&filter[status]=published&sort[translations.name]=sort&limit=-1`;
  return req(url);
};

export const getStaticData = async (loc) => {
  const locale = languagesApi[loc];
  const url = `static_data?fields=tag_manager_id,google_site_ver,no_transport,captcha_enabled,google_satellite_map,debug_mode,logo,telegram_banner,telegram_banner_x2,telegram_banner_x_3,translations.filter_value,translations.name&deep[translations][_filter][languages_code][_eq]=${locale}&filter[status]=published`;
  return req(url);
};

export const getAllToursTextPages = async (loc = languagesApi.ru) => {
  const locale = languagesApi[loc];
  const url = `tours_text?fields=slug,subpage,subsubpage,img,popular,time_of_year,translations.languages_code,translations.name&deep[translations][_filter][languages_code][_eq]=${locale}&filter[status]=published&limit=-1`;
  return req(url);
};

export const getToursTextPage = async (loc, slug, subpage = null, subsubpage = null) => {
  const locale = languagesApi[loc];
